...
<main class="container-fluid px-4 py-4" style="background-color: #FCFDF9;">
    <div class="mb-3">
        <?php
            $backUrl = '?';
            $backParams = [];
            // Giữ lại controller & action (nếu có) để không làm hỏng router của nhóm
            if(isset($_GET['controller'])) $backParams['controller'] = $_GET['controller'];
        ...
        <div class="col-lg-3 mb-4">
            <div class="sidebar-filter">
                ...
                <form action="" method="GET" id="filterForm">
                    <!-- Giữ lại thông tin controller trên url nếu hệ thống của nhóm bạn dùng index.php?controller=... -->
                ...
                <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3">
                    ...
                    <form method="GET" class="d-flex align-items-center" action="">
                        ...
                        <select name="sort" id="sort" class="form-select form-select-sm rounded-pill shadow-sm" style="width: auto;" onchange="this.form.submit()">
                            ...
                            <option value="moi_nhat" <?php echo $sort == 'moi_nhat' ? 'selected' : ''; ?>>Mới nhất</option>
                                ...
                                <div class="card tour-card border-0">
                                    ...
                                    <div class="tour-card-body bg-white">
                                        <div class="text-danger small mb-1 fw-bold text-uppercase">
                                            <i class="fa-solid fa-location-dot me-1"></i> <?php echo htmlspecialchars($tour['VungDiaLy']); ?>
                                            <?php if(!empty($tour['DiaDiem'])): ?>
                                                <span class="text-secondary fw-semibold text-capitalize" style="font-size: 0.9em; letter-spacing: 0;"> - <?php echo htmlspecialchars($tour['DiaDiem']); ?></span>
                                            <?php endif; ?>
                                        </div>
            ...
            <?php else: ?>
                <div class="mb-5">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <a href="?<?php echo isset($_GET['controller']) ? 'controller='.$_GET['controller'].'&' : ''; ?>view=latest" class="text-warning text-decoration-none small fw-bold">Xem thêm ></a>
                    </div>
                    <div class="row g-4">
                ...
                <div class="mb-5 bg-white p-4 rounded-4 shadow-sm border">
                    ...
                    <div class="carousel-container">
                        ...
                        <div class="carousel-track" id="danhMucTrack">
                            ...
                            <a href="?<?php echo isset($_GET['controller']) ? 'controller='.$_GET['controller'].'&' : ''; ?>loai[]=<?php echo urlencode($name); ?>" class="carousel-item-box text-decoration-none">
                ...
                <div class="mb-5 bg-white p-4 rounded-4 shadow-sm border">
                    ...
                    <div class="carousel-container">
                        ...
                        <div class="carousel-track" id="vungTrack">
                            ...
                            <a href="?<?php echo isset($_GET['controller']) ? 'controller='.$_GET['controller'].'&' : ''; ?>vung=<?php echo urlencode($name); ?>" class="carousel-item-box text-decoration-none">
                                <img src="<?php echo $img; ?>" alt="<?php echo $name; ?>">
                                <div class="carousel-overlay">TOUR <?php echo mb_strtoupper($name, 'UTF-8'); ?></div>
                            </a>
                            <?php endforeach; ?>
...
<script>
    ...
    // Xử lý Thả tim (Chờ AJAX server xử lý xong mới đổi màu thay vì đổi "ảo")
    function toggleHeart(element, maTour) {
        const icon = element.querySelector('i');
        const countSpan = element.querySelector('.like-count');
        let count = parseInt(countSpan.innerText);

        // Hiển thị trạng thái đang xử lý để user không bấm liên tục
        ...
        .then(data => {
            ...
            if (data.success) {
                // Đã lưu Database thành công -> Đổi màu giao diện
                ...
                // Server báo lỗi -> Hiện thông báo
                console.error("Lỗi từ server: ", data.message);
                alert("Lỗi thả tim: " + data.message + "\n(Vui lòng kiểm tra lại cấu hình Database trong Controller)");
            }
        })
        .catch(e => {
            // Lỗi kết nối
            ...
            alert("Lỗi kết nối đến server! Không thể lưu dữ liệu.");
        });
    }
</script>