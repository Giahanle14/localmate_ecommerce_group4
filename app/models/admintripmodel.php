...
class AdminTripModel {
    ...
    public function getTripStats() {
        $stats = [
            ...
            'pending' => $this->conn->query("SELECT COUNT(cd.MaChuyenDi) FROM ChuyenDi cd JOIN LichKhoiHanh lkh ON cd.MaLichKhoiHanh = lkh.MaLichKhoiHanh JOIN Tour t ON lkh.MaTour = t.MaTour WHERE cd.TrangThai != 'Đã hủy' AND DATE_ADD(lkh.NgayBatDau, INTERVAL t.SoNgay DAY) >= CURDATE()")->fetchColumn(),
            'completed' => $this->conn->query("SELECT COUNT(cd.MaChuyenDi) FROM ChuyenDi cd JOIN LichKhoiHanh lkh ON cd.MaLichKhoiHanh = lkh.MaLichKhoiHanh JOIN Tour t ON lkh.MaTour = t.MaTour WHERE cd.TrangThai != 'Đã hủy' AND DATE_ADD(lkh.NgayBatDau, INTERVAL t.SoNgay DAY) < CURDATE()")->fetchColumn(),
    ...
    public function getTrips($tab, $search, $daterange, $page, $limit) {
        $sql = "SELECT c.MaChuyenDi, lkh.NgayBatDau, DATE_ADD(lkh.NgayBatDau, INTERVAL t.SoNgay DAY) as NgayKetThuc, c.TrangThai, 
                       tk.HoTen, t.TenTour, 
        ...
        if ($tab === 'pending') {
            $sql .= " AND c.TrangThai != 'Đã hủy' AND DATE_ADD(lkh.NgayBatDau, INTERVAL t.SoNgay DAY) >= CURDATE()";
        ...
        if (!empty($daterange)) {
            ...
            if (!empty($matches[1])) {
                ...
                if ($startObj && $endObj) {
                    ...
                    $sql .= " AND (lkh.NgayBatDau >= :start AND DATE_ADD(lkh.NgayBatDau, INTERVAL t.SoNgay DAY) <= :end)";
    ...
    public function getTripDetail($maChuyenDi) {
        $sql = "SELECT c.*, 
                       ...
                       lkh.NgayBatDau, DATE_ADD(lkh.NgayBatDau, INTERVAL t.SoNgay DAY) as NgayKetThuc,
